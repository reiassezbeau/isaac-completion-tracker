...
import { VIEW_STRINGS_6 } from "./i18n-views6";
...
export function translate(key: string, lang: Lang): string {
  const e = T[key] ?? VIEW_STRINGS[key] ?? VIEW_STRINGS_2[key] ?? VIEW_STRINGS_3[key] ?? VIEW_STRINGS_4[key] ?? VIEW_STRINGS_5[key] ?? VIEW_STRINGS_6[key];
  if (!e) return key;
