  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <h1 className="flex items-center gap-2 font-display text-3xl text-isaac-text">
        <ShieldAlert className="h-6 w-6 text-isaac-gold" /> {t("nav.settings")}
      </h1>
      ...
      <div className="flex items-start gap-3 rounded-xl border border-isaac-border bg-isaac-surface px-4 py-3 text-sm">
        ...
        <div className="text-isaac-muted">
          {t("set.intro")}<strong className="text-isaac-text">{t("set.neverInSave")}</strong>{t("set.appliedOver")}
        </div>
      </div>

      <div className="flex items-center justify-between">
        <span className="text-sm text-isaac-muted">
          {overrideCount} {t("set.activeCount")}
        </span>
        <button
        ...
        >
          <RotateCcw className="h-4 w-4" /> {t("set.resetBtn")}
        </button>
      </div>
            ...
            return (
              <div key={a.id} className="flex items-center justify-between gap-2 rounded-lg border border-isaac-border bg-isaac-surface2/40 px-3 py-2 text-sm">
                <span className="flex items-center gap-2">
                  {a.name}
                  {a.overridden && <Pill className="border-isaac-gold/40 bg-isaac-gold/10 text-isaac-gold">{t("set.fixed")}</Pill>}
                  <span className="text-xs text-isaac-muted">({a.unlocked ? t("set.unlocked") : t("set.locked")})</span>
                </span>
                <div className="flex gap-1">
                  <button onClick={() => setAchOverride(a.id, true)} className={`rounded px-2 py-1 text-xs ${forced === true ? "bg-isaac-done/20 text-isaac-done" : "bg-isaac-surface text-isaac-muted hover:text-isaac-text"}`}>{t("set.unlocked")}</button>
                  <button onClick={() => setAchOverride(a.id, false)} className={`rounded px-2 py-1 text-xs ${forced === false ? "bg-isaac-blood/20 text-isaac-blood" : "bg-isaac-surface text-isaac-muted hover:text-isaac-text"}`}>{t("set.locked")}</button>
                  <button onClick={() => setAchOverride(a.id, null)} className="rounded px-2 py-1 text-xs text-isaac-muted hover:text-isaac-text">{t("set.auto")}</button>
                </div>
              </div>
            );
          })}
          {q && results.length === 0 && <p className="text-sm text-isaac-muted">{t("set.noAch")}</p>}
        </div>
      </Card>
            ...
            return (
              <div key={e.id} className="flex items-center justify-between gap-2 rounded-lg border border-isaac-border bg-isaac-surface2/40 px-3 py-2 text-sm">
                ...
                <div className="flex gap-1">
                  ...
                  <button onClick={() => setMarkOverride(e.mark_index, null)} className="rounded px-2 py-1 text-xs text-isaac-muted hover:text-isaac-text">{t("set.auto")}</button>
                </div>
              </div>
            );
          })}
      ...
      >
        <strong className="text-isaac-text">{overrideCount}</strong> {t("set.confirmBody")}
      </Modal>
    </div>
  );
}
