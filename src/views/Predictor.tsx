          ...
          <div className="mt-4 space-y-3">
            ...
            {result.already_unlocked.length > 0 && (
              <div className="text-xs text-isaac-muted">
                {t("pred.already")} {result.already_unlocked.map((a) => a.name).join(", ")}
              </div>
            )}
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              {result.advances_dead_god ? (
                <span>
                  {t("pred.bringsCloser")} <strong className="text-isaac-gold">Dead God</strong> —{" "}
                  {result.target_name} : {t("pred.markNow")} <em>{result.current_mark}</em>, {t("pred.toHard")}
                </span>
              ) : (
                <span className="text-isaac-muted">{t("pred.alreadyHard")}</span>
              )}
            </div>
          </div>
        )}
        <p className="mt-4 flex items-center gap-1.5 text-xs text-isaac-muted">
          <Info className="h-3 w-3" /> {t("pred.note")}
        </p>
      </Card>

      <Card>
        <SectionTitle hint={t("pred.byUnlocks")}>{t("pred.whatNext")}</SectionTitle>
        {best.length === 0 ? (
          <EmptyState title={t("pred.nothing")} />
        ) : (
          <div className="space-y-2">
            {best.map((sg, i) => (
              <button
                key={i}
                onClick={() => {
                  setCharId(sg.character_id);
                  setTargetId(sg.target_id);
                }}
